...
class ManagedPage extends Page
{
    ...
    public function parseError($code, $data = null)
    {
        if($code == ResponseCode::CODE_404){
            $this->setResult($this->managedFactory()->appTemplates()->load('ManagedModule:404.html'), ContentType::TYPE_HTML_UTF8, ResponseCode::CODE_404);
        }else{
            $this->setResult($this->managedFactory()->appTemplates()->load('ManagedModule:500.html'), ContentType::TYPE_HTML_UTF8, ResponseCode::CODE_500);
        }
